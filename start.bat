...
powershell -NoProfile -WindowStyle Hidden -Command "$env:PYTHONUTF8='1'; Get-CimInstance Win32_Process -Filter 'name=''python.exe''' | Where-Object { $_.CommandLine -like '*recorder.py*' -or $_.CommandLine -like '*dashboard_server.py*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }; Start-Sleep -Seconds 2; Start-Process -FilePath '%PY%' -ArgumentList 'recorder.py' -WorkingDirectory '%DIR%' -WindowStyle Hidden -PassThru; Start-Process -FilePath '%PY%' -ArgumentList 'dashboard_server.py' -WorkingDirectory '%DIR%' -WindowStyle Hidden -PassThru; $obs = '$env:LOCALAPPDATA\Programs\obsidian\Obsidian.exe'; if (Test-Path $obs) { $running = Get-Process -Name 'Obsidian' -ErrorAction SilentlyContinue; if (-not $running) { Start-Process -FilePath $obs } }"
